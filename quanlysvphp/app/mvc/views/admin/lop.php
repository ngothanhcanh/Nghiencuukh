                        ...
                        <div class="adv-table editable-table ">
                            <div class="clearfix">
                                ...
                                <div class="btn-group pull-right">
                                    ...
                                    <li>
                                            <form method="POST" action="<?= URL ?>/AdminLopController/import" enctype="multipart/form-data">
                                                <input type="file" id="excelFile" name="excelFile" accept=".xlsx" onchange="checkFileSelected()">
                                                <button type="submit" name="importlop" id="importButton" disabled>Import</button>
                                            </form>
                                        </li>
                                        <form method="POST" action="<?= URL ?>/AdminLopController/export">
                            ...
                            <div id="editable-sample_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                <div class="row">
                                    ...
                                    <div class="col-lg-6">
                                        <div class="dataTables_filter" id="editable-sample_filter"><label>Search: <input type="text" id="search-input" aria-controls="editable-sample" class="form-control medium"  placeholder="nhập tên lớp để tìm kiếm.."></label></div>
                                    </div>
                                </div>
                                <table class="table table-striped table-hover table-bordered dataTable" id="editable-sample" aria-describedby="editable-sample_info">
                                    <thead>
                                        <tr role="row">
                                            <th class="sorting_disabled" role="columnheader" rowspan="1" colspan="1" aria-label="First Name" style="width: 120px;">Mã lớp</th>
                                            <th class="sorting" role="columnheader" tabindex="0" aria-controls="editable-sample" rowspan="1" colspan="1" aria-label="Last Name: activate to sort column ascending" style="width: 223px;">Tên lớp</th>
                                    ...
                                    <tbody id="search-results" role="alert" aria-live="polite" aria-relevant="all">
                                        <tr id="new-row" style="display:none;">
                                            ...
                                            <td contenteditable="true" id="newPassword">
                                            <select class="makh-select">
                                                    <option value="">Không</option>
                                                    <?php foreach ($result_KhoaModel as $rowkhoa) { ?>
                                                        <option value="<?php echo $rowkhoa['MAKH'] ?>"><?php echo $rowkhoa['MAKH'] ?></option>
                                                    <?php } ?>
                                             </select>
                                            </td>
                                            <td><button id="saveButton" class="save">save</button></td>
                                            <td><a class="delete" name="delete" href="<?= URL ?>/AdminLopController/index?delete=">Delete</a></td>
                                        </tr>
                                        <?php foreach ($result as $row) {
                                        ?>
                                            <tr class="odd" id="<?= $row['MALOP'] ?>">
                                                <td class="sorting_1"><?php echo $row['MALOP'] ?></td>
                                                <td class="name"> <?= $row['TENLOP'] ?></td>
                                                <td class="password"><?= $row['MAKH'] ?></td>
                                                <td class="edit"><a class="edit-btn" name="edit" href="#">Edit</a></td>
                                                <td class=" "><a class="delete" name="delete" href="<?= URL ?>/AdminLopController/index?delete=<?= $row['MALOP'] ?>">Delete</a></td>
                                            </tr>
                                        <?php
                                        }
                                        ?>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.edit-btn', function() {
            ...
            row.find('.password').html('<select class="editlop"><option value="">Không</option><?php foreach ($result_KhoaModel as $rowkhoa) { ?><option value="<?php echo $rowkhoa['MAKH'] ?>"><?php echo $rowkhoa['MAKH'] ?></option><?php } ?></select>')
            ...
            row.find('.update-btn').on('click', function() {
                ...
                var editedPassword = row.find('.editlop').find('option:selected').val();
                var data={
                        id:id,
                ...
                $.ajax({
                url: '<?= URL ?>/AdminLopController/update',
                type: 'POST',
                dataType: 'json',
                data: data,
        ...
        $('#search-input').on('input', function() {
            ...
            <?php if (isset($result)) {
                foreach ($result as $row) { ?>
                    var name = '<?php echo $row['TENLOP']; ?>'.toLowerCase(); //đặt biến name là tên của giá trị name trong bảng người dùng
                    if (name.includes(searchValue)) //so sách giá trị tìm bằng giá trị name
                    {
                        var listItem = '<tr class="odd" id="<?= $row['MALOP'] ?>"><td class="sorting_1"><?php echo $row['MALOP'] ?></td><td class="name"> <?= $row['TENLOP'] ?></td><td class="password"><?= $row['MAKH'] ?></td><td class="edit"><a class="edit-btn" name="edit" href="#">Edit</a></td><td class=" "><a class="delete" name="delete" href="<?= URL ?>/AdminLopController/index?delete=<?= $row['MALOP'] ?>">Delete</a></td></tr>';
        ...
        $(document).on('click', '.save', function(e) {
            ...
            var password = newRow.find('.makh-select').val();
            ...
            $.ajax({
                url: '<?= URL ?>/AdminLopController/save',
                type: 'POST',
                dataType: 'json',
                data: data,
                success: function(response) {
                ...
                <tr>
                    ...
                    <td><a class="edit" name="delete" href="<?= URL ?>/AdminLopController/index?delete=${response.id}">Delete</a></td>
                </tr>
                  `;
