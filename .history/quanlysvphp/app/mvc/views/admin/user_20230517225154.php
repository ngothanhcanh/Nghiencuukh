                            ...
                            <div id="editable-sample_wrapper" class="dataTables_wrapper form-inline" role="grid">
                                ...
                                <table class="table table-striped table-hover table-bordered dataTable" id="editable-sample" aria-describedby="editable-sample_info">
                                    ...
                                    <tbody id="search-results" role="alert" aria-live="polite" aria-relevant="all">
                                        ...
                                        <?php if (isset($result) && is_array($result) || is_object($result)) { foreach ($result as $row) {
                                        ?>
                                            <tr class="odd">
...
<script>
     ...
     $('#search-input').on('input', function() {
        ...
        <?php if (isset($result) && is_array($result)) { foreach($result as $row) { ?>
            ...
            {
                var listItem = '<tr class="odd"><td class="  sorting_1"><?php echo $row['ID'] ?></td><td class=" "> <?= $row['name'] ?></td> <td class=" "><?= $row['password'] ?></td><td class="center "><?= $row['user_type'] ?></td><td class="center "><?= $row['status'] ?></td><td class="center "><?= $row['MSSV'] ?></td><td class="center "><?= $row['MAGV'] ?></td><td class=" "><a class="edit editUser" name="edit" href="<?= URL ?>/UserController/index?edit=<?= $row['ID'] ?>">Edit</a></td><td class=" "><a class="delete" name="delete" href="<?= URL ?>/UserController/index?delete=<?= $row['ID'] ?>">Delete</a></td></tr>';
     ...
     $(document).on('click', '#updateButton', function() {
         var newRow = $(this).closest('tr'); 
         var id = newRow.find('.idUser').text();
         var name = newRow.find('.nameUser').text();
         var password = newRow.find('.passUser').text();
         var userType = newRow.find('.inp-usertype').val();
         var status = newRow.find('.inp-status').val();
         var mssv = newRow.find('.mssv-inp').val();
          var magv = newRow.find('.magv-inp').val();
         var data = {
             id: id,
             name: name,
             password: password,
         ...
         $.ajax({
             ...
             data: data,
             success: (response) => {
                 $(this).parent("td").html(`<a class="editUser" href="#">edit</a>`)
                 newRow.find('.idUser').text(id)
                 newRow.find('.nameUser').text(name)
                 newRow.find('.passUser').text(password)
                 newRow.find('.inp-usertype').text(userType)
                 newRow.find('.inp-status').text(status)
                 newRow.find('.mssv-inp').text(mssv)
                 newRow.find('.magv-inp').text(magv)
             },
             error: function(xhr, status, error) {
                 console.error(error);
             }
